const getInfo = () => {
  fetch(
    "https://restcountries.com/v2/all?fields=name,languages,currencies,population,area,borders"
  )
    .then((res) => res.json())
    .then((data) => {
      let countriesArr = [];
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        data[i].borders?.forEach((border) => neighboursArr.push(border));
        data[i].currencies?.length !== 0
          ? data[i].currencies?.forEach((el) => currencyArr.push(el.code))
          : currencyArr === "";
      }
